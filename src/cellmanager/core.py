...
class CellNode(Node):
    ...
    def filter_markers(self, markers: list[str]):
        self.markers = sorted([marker for marker in self.markers if marker in markers])
...
class CellManager:
    ...
    def render_level_table(self,):
        ...
        self.level_order_group = [[child for child in children if child.name != "#"] for children in LevelOrderGroupIter(self.cell_tree)]
    ...
    def render_cluster_table(self, cluster: str):
        ...
        table.add_column("Cell type", justify="center")
        table.add_column("Markers", justify="center", max_width=80)
        for node in self.find_children(name=cluster):
            table.add_row(Text(node.name, style=Style(bold=True, color=node.color)), Text(", ".join(node.markers)))
            table.add_section()
        console = Console()
        console.print(table)
    ...
    def query(self, by: Literal["level", "cluster", "list"], key: str | list[str], info: Literal["color", "markers", "both"] = "markers", output_format: Literal["dict", "list"] = "dict", include_major: bool = False):
        if by == "level":
            ...
            if major_cell_type:
                children = self.find_children(parent=major_cell_type)
                data = children if not include_major else [major_cell_type] + children
            else:
                raise ValueError(f"Cluster {key} not found")      
    ...
    def find_children(self, name: str | None = None, parent: CellNode | None = None) -> list[CellNode]:
        assert name or parent, "Please specify either `name` or `parent`"
        node = parent if parent else find(self.cell_tree, lambda x: x.name == name)
        if node:
            ...
            raise ValueError(f"Cell type {name} not found")
